...
- 现象（用户报告）：插件面板中 @xmanrui/dsh-im 4.21.1 与 dshmarket 1.47.0 两条记录
  永久显示「dsh 安装目录缺失（GitHub shim 指向的目录已不存在或为空），请重新安装
  dsh」，而实际上安装目录存在、dsh 运行正常。
- 取证：`%APPDATA%\dsh-launcher\plugins.json` 中两条记录的 lastError 由
  2026-09-16 10:21 写入——那是换通道安装的清空窗口（`清理旧目录` 已执行、新目录
  尚未 clone 完成），此刻同步失败对每个插件写入该错误；10:29 重装完成后目录恢复、
  dsh 全程可用，但该错误**没有任何清除路径**（只有「再次同步成功」或「再次操作
  该插件」才会清），于是面板永久显示已不成立的错误，误导用户重复安装 dsh。
- BUG-A：`list()` 对账时重验环境类陈旧错误——若当前 `resolve_entry()` 成功
  （dsh 可用），清除携带 DshNotInstalled 语义的 lastError 并落盘（`is_environment_error`
  以本仓库固定文案常量为判据，插件自身错误不受影响）。
- BUG-B：`sync` 失败按类别分流——环境类错误（dsh/node 不可用）不再写入每个插件的
  lastError（那会把一次全局故障放大成 N 条与插件无关的错误），只保留在结果消息与
  日志中；插件自身失败（安装/校验）照旧记录。
- 修复顺序缺陷（由新增端到端测试抓出）：`build_views` 原先在注册表清理之前执行，
  导致清除当次的响应仍携带旧 lastError；已把视图构建移到全部清理/清除之后，
  保证面板一次刷新即显示最新状态。
- 测试：新增 `tests/stale_error_e2e.rs`（真实 profile + 真实注册表：注入两条实测
  陈旧错误 → list 清除 → 落盘确认 → 二次 list 幂等 → 还原现场）；lib 单测新增
  `is_environment_error` 判定（4 个命中样本 + 3 个不命中样本），223/223 通过。
- 验证：`cargo check --all-targets` 0 错误 0 警告；`tsc --noEmit` 干净；
  `npm run build` 成功；版本一致性 0.9.9。
