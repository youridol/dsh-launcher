...
- 🔀 **版本随意切、随意退** —— 版本管理面板把 npm 与 GitHub 两条通道的**历史版本全列出来**（最新置顶），
  想升就升、想退就退：点哪个版本就装哪个，比如从最新版一键**退回**到某个稳定版、或换一条通道再装。
  不用记 `npm i -g xxx@版本`、不用自己 `git clone --branch <tag>` 再编译；
  **切版本/换通道也不会动 `~/.dsh` 里的会话、技能与插件配置**（详见下方「🔒」一节）。
